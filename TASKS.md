...
| D2.17 | **Distance-adaptive low-battery RTL trigger** | **6 h** | Deferred from 0.2 (David 2026-08-11): SBC (or FC Lua script) computes energy-needed-to-return from the current GPS distance-to-home each cycle and commands RTL via MAVLink at ~10% margin, instead of waiting for the static `R6_FS_BATT` worst-case (2.8 km) threshold — avoids cutting a mission short when the drone is already near home with reserve to spare. `BATT_CRT_MAH` stays a firmware-level hard-floor backstop regardless. See MODEL_ISSUES.md §B8 and Mission Control task #142. [claude-nightly 2026-08-12] Reference implementation (haversine distance, pack-energy accounting, live cruise-power EMA) built + 15 unit tests, standalone in the Mission-Control repo (not yet wired into this repo). [claude 2026-08-13] Ported `energy_model.py`/`battery_rtl_trigger.py`/`test_battery_rtl_trigger.py` into `analysis/autonomy_sim/`; wired `BatteryRTLMonitor` into `MissionApp` (`_pump()` now handles `HOME_POSITION`/`BATTERY_STATUS`/`PARAM_VALUE` for `RTL_SPEED`→`WPNAV_SPEED` fallback→`RTL_ALT`, all read live rather than hardcoded); extended `FakeFC` to emit both and answer `PARAM_REQUEST_READ`; added 2 integration tests proving the far-triggers/near-doesn't-trigger contract over real MAVLink messages, not just the pure-math layer. Full suite (20 tests) green, 3 consecutive runs. **Still open:** `climb_energy_j` stays the placeholder `0.0` (conservative-safe direction, but not a real climb-rate measurement) and this has **not been validated against real ArduPilot SITL** (only the `FakeFC` mock) — that's the genuine remaining item, now unblocked by D2.16's SITL fixes. | ◐ |
