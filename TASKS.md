...
| D1.3 | Labeling + **augmentation** pipeline | **8 h** | Label→class mapping ✅; augmentation pipeline built: `analysis/thermal_sim/augmentation_pipeline.py` (albumentations + OpenCV fallback, altitude-band-aware, mosaic, 20 unit tests). [agent 2026-08-19] Full code written and tested. Awaiting albumentations install verification + DroneThermalModel integration. | ◐ |
...
| D2.12 | Service hardening — systemd unit, watchdog, logging, config, safe stand-down | **6 h** | Field-ops requirement (single operator). [agent 2026-08-14] Built the complete deliverable in `analysis/service_hardening/`: `README.md` (architecture + deployment guide), `mission_app.service` (systemd unit with WatchdogSec=30, Restart=always, NoNewPrivileges/ProtectSystem/PrivateTmp), `mission_app_config.yaml` (50+ params: MAVLink, mission, battery RTL, thermal cam, inference, logging, system), `mission_app_logging.py` (JSON/text structured logging, RotatingFileHandler, journald forwarding), `run_service.py` (~450 lines: YAML config loader, sd_notify watchdog, SafeStandDown daemon thread on separate conn for crash recovery, SIGTERM/SIGINT graceful LAND, SITL mode flag). Verified both Python modules compile and logger produces correct JSON/text output. | ◐ |
