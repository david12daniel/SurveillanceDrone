...
| **Thermal camera** | **PurpleRiver Mini 640** (640×512, 12 µm, **18 mm lens**, USB) | [`T13`](https://www.thermal-image.com/product/mini-640-uncooled-lwir-thermal-camera-module/) | 18mm updated 2026-07-29 | `FIXED_THERMAL_ID = "T13"` | 18 mm (was 13 mm): **4.17 px on 0.5 m @90 m** at the as-built 45° mount tilt (along-range axis, thin but passing the 4 px floor; nadir would read 8.3 px — see `model.sysml` `OffNadirGsd`, 2026-08-12) + 3.13 px detect @120 m, and is the shortest lens that **recognizes at a 45° oblique tilt**. HFOV **24.1°** → **51.2 m ground swath** at 120 m AGL. IR is **surveillance-only, not used to fly** (the FPV cam does). See [`analysis/thermal_detection_offnadir_analysis.md`](analysis/thermal_detection_offnadir_analysis.md). **RESOLVED 2026-08-11:** R3_CAM_FOV was restated from "≥30° HFOV" to a **≥42 m ground-swath floor** derived from the new mission requirements **R9** (hard: ≥30 acres/sortie) and **R10** (stretch: 60 acres) — the selected lens now **passes with 22% margin**. MODEL_ISSUES.md §B7. | **2** |
