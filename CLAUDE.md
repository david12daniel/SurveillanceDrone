...
**Holistic sweep (23,184 raw pairings → 14,112 interface-compatible "real" configs):** fully crosses the flight-time drivers — airframe × battery × SBC × VTX × thermal camera — holds the sub-1 W peripherals (FPV cam, GPS, RX) at lightest representatives, then **filters out incompatible pairings** per the model's `Compatibility` rules: **P1** battery↔airframe cell-count (drops e.g. a 4S pack on a 6S-only frame; 7,056 configs) and **V2** thermal↔DVR (a thermal no DVR can record — raw SPI/CMOS — is dropped; 2,016 configs). The **DVR is not crossed and is excluded from flight time**: it is an earlier-stage (Phase 1-3) part, so the headline endurance is the SBC-stage build (SBC present, DVR removed); the DVR is kept only for earlier-stage camera↔DVR compatibility and for cost. Falls back to a generated generic LiPo/Li-ion grid only if no Battery candidates exist in `candidates.sysml`. 14 of 15 airframes participate (AF5/EMAX Hawk 7 still lacks a confirmed mass). **Recommended baseline:** KOLAS7 (AF2a) + 6S 12Ah Amprius → 74.6 min hover (the earlier 4S 89.2 min "winner" was a fake — KOLAS7 is 6S-only). See `MODEL_ISSUES.md` §C11-C12.
